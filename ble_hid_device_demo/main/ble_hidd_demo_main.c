...
static bool jiggleForth = false;
static bool enJiggle = false;
...
static void hidd_event_callback(esp_hidd_cb_event_t event, esp_hidd_cb_param_t *param)
{
    switch(event) {
        ...
        case ESP_HIDD_EVENT_BLE_LED_REPORT_WRITE_EVT: {
            ...
            if(param->led_write.data[0] & 0x4) enJiggle = true;
            else enJiggle = false;
            break;
        }
        default:
...
void animateAndWait(uint32_t color)
{
    struct led_state newState;
    for (uint8_t iLed=0; iLed<CONFIG_WS2812_NUM_LEDS; ++iLed) newState.leds[iLed] = color;
...
void hid_demo_task(void *pvParameters)
{
    ...
    ws2812_control_init();
    while(1) {
        animateAndWait(color);
        if (sec_conn) {
            // BLE connected
            if (enJiggle) {
                ESP_LOGI(HID_DEMO_TAG, "jiggle");
                esp_hidd_send_mouse_value(hid_conn_id, 0, jiggleForth ? 1 : -1, 0);
                color = jiggleForth ? COLOR_JIGGLE_FORTH : COLOR_JIGGLE_BACK;
                jiggleForth = !jiggleForth;
            } else {
                color = COLOR_JIGGLE_NOT;
            }
        } else {
            // BLE disconnected
            color = COLOR_DISCONNECTED;
        }
        if (gpio_get_level(BUT_USR_GPIO)) {
            // user button pressed
            uint8_t cmd[1] = {HID_KEY_SCROLL_LOCK};
            vTaskDelay(10 / portTICK_PERIOD_MS); // debouncing
            esp_hidd_send_keyboard_value(hid_conn_id, 0, cmd, 1);
            while(gpio_get_level(BUT_USR_GPIO)) vTaskDelay(10 / portTICK_PERIOD_MS);
